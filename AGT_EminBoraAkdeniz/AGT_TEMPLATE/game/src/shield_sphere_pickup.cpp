...
shield_sphere_pickup::shield_sphere_pickup(glm::vec3 start_colour, glm::vec3 end_colour, bool zap, float zap_level, float max_time)
{
...
shield_sphere_pickup::~shield_sphere_pickup()
{}

void shield_sphere_pickup::on_update(const engine::timestep& time_step)
{
    if (!s_active)
        return;

    // Program the object to get larger and change colour over time
    m_scale = 1.5f;
...
void shield_sphere_pickup::on_render(engine::ref<engine::shader> shader)
{
    if (!s_active)
        return;
...
void shield_sphere_pickup::activate(float radius, glm::vec3 position)
{
    s_active = true;
    m_position = position;
    m_timer = 0.0f;
...
engine::ref<shield_sphere_pickup> shield_sphere_pickup::create(glm::vec3 start_colour, glm::vec3 end_colour, bool zap, float zap_level, float max_time)
{
    return std::make_shared<shield_sphere_pickup>(start_colour, end_colour, zap, zap_level, max_time);
}

// Return a random floating point number between 0 and 1
float shield_sphere_pickup::random()
{
    return (rand() / (float)RAND_MAX) * 2.0f - 1.0f;
}

void shield_sphere_pickup::set_position(glm::vec3 pos) {
    m_position = pos;
}
